...
fn key_pressed(app: &App, model: &mut Model, key: Key) {
    bleh::key_pressed(app, &mut model.field_up, &mut model.field_left, key);
    match key {
...
fn model(app: &App) -> Model {
    ...
    let mut balls: Vec<Ball> = (0..COUNT)
        .map(|x| Ball::new(ball_size(x, COUNT), ball_color(x, COUNT, 0.1, 0.1)))
        .collect();

    let mut new_balls: Vec<Ball> = (0..COUNT)
        .map(|x| Ball::new(ball_size(x, COUNT), ball_color(x, COUNT, 0.5, 0.1)))
        .collect();

    balls.append(&mut new_balls);

    let mut new_balls: Vec<Ball> = (0..COUNT)
        .map(|x| Ball::new(ball_size(x, COUNT), ball_color(x, COUNT, 0.8, 0.1)))
        .collect();
